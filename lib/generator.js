...
const { getRepoList, getRepoBranches } = require('./http')
/**
 * 处理项目创建逻辑
 */
class Generator {
  ...
  async getRepo () {
    // 获取配置的gitlab groupId
    const groupId = 72|| getGitlabGroupId()
  ...
  async download (repoId, sha) {
    const token = 'DwJKzKBESJYDegSpGA5K'|| getToken()
    const baseUrl ='http://36.133.80.161:8091'|| getGitlabBaseUrl()
